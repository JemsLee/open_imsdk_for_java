...
import com.momo.imc.net.IMNetworkUtil;
...
public class IMManager {
    ...
    public void startSocket(){
        ...
        try {
            ...
            imWebSocketClient.fbFlag = fbFlag;
            if(needAck != null){
    ...
    private void startCheckNetWork(){
        scheduledExecutor.scheduleWithFixedDelay(new TimerTask() {
            @Override
            public void run() {
                boolean nRs = IMNetworkUtil.isNetWorkAvailable();
                if(!nRs){
                    netWorkStatus = 1;
                    isRuning = false;
                }
                if(nRs && netWorkStatus == 1){
                    netWorkStatus = 0;
                    imWebSocketClient.reconnect();
                    isRuning = true;
                }
...
//        IMLogger.getInstance().info("ping的消息"+jsonObject.toJSONString());
